...
- **H1: append latency < 25 ms p95 at 10^3 commits, flat in context size.**
  ...
  real session (fanout layout; the flat layout degraded to 88 ms). Next:
...
1. **Recoverable vs. lossy compaction** (headline experiment). Long-horizon
   ...
   (c) recovers most planted facts; (a)/(b) approach zero as distance grows.
...
3. **Replay determinism.** Re-run the LLM against a historical commit's
   exact context with pinned tool results; measure how often behavior
...
- pygit2 backend (sub-ms appends); tuned `repack` policy as part of the
  store, since default gc leaves 2–3x on the table for this workload
- `memory_search` as a real agent tool; wire into a live agent loop
  (e.g. Claude Agent SDK) recording actual sessions
- embedding sidecar keyed by blob SHA (grep-based `grep_history` does not
  scale to 72k blobs; an index is mandatory at this size)
